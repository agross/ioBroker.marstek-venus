...
const Polling = {
	...
	async pollSlow() {
		...
		try {
			if (this.config.enableWifiStatus !== false) {
				await this.pollWifiStatus();
			} else {
				this.log.debug("Skipping Wifi.GetStatus - disabled in config");
			}
			if (this.config.enableBLEStatus !== false) {
				await this.pollBLEStatus();
			} else {
				this.log.debug("Skipping BLE.GetStatus - disabled in config");
			}
		} finally {
	...
	async pollWithRetry(fn, maxRetries = 1) {
		let lastError;
	...
	async poll() {
		...
		try {
			...
			if (this.config.enableBatteryStatus !== false) {
				pollTasks.push(this.pollWithRetry(() => this.pollBatteryStatus()));
			}
			if (this.config.enableEMStatus !== false) {
				pollTasks.push(this.pollWithRetry(() => this.pollEMStatus()));
			}
			if (this.config.enableModeStatus !== false) {
				pollTasks.push(this.pollWithRetry(() => this.pollModeStatus()));
			}
			if (this.config.enablePVStatus !== false && this.hasPVSupport()) {
				pollTasks.push(this.pollWithRetry(() => this.pollPVStatus()));
			} else if (this.config.enablePVStatus !== false && !this.hasPVSupport()) {
				this.log.debug("Skipping PV polling - device does not support PV");
			} else {
				this.log.debug("Skipping PV polling - disabled in config");
			}

			if (pollTasks.length === 0) {
				this.log.debug("No normal poll endpoints enabled, skipping poll cycle");
				return;
			}
	...
	async pollESStatus() {
		try {
			...
			if (result.total_grid_output_energy !== undefined && result.total_grid_output_energy !== null) {
				await this.setStateChangedAsync("energy.gridExport", {
					val: result.total_grid_output_energy,
					ack: true,
				});
			}
			if (result.total_grid_input_energy !== undefined && result.total_grid_input_energy !== null) {
				await this.setStateChangedAsync("energy.gridImport", {
					val: result.total_grid_input_energy,
					ack: true,
				});
			}
	...
	async pollPVStatus() {
		...
		if (result.pv1_power !== undefined && result.pv1_power !== null) {
			const pv1_corr = Math.trunc(result.pv1_power / 10);
			await this.setStateChangedAsync("power.pv1", { val: pv1_corr, ack: true });
		}
		if (result.pv1_voltage !== undefined && result.pv1_voltage !== null) {
			await this.setStateChangedAsync("power.pv1Voltage", { val: result.pv1_voltage, ack: true });
		}
		if (result.pv1_current !== undefined && result.pv1_current !== null) {
			await this.setStateChangedAsync("power.pv1Current", { val: result.pv1_current, ack: true });
		}
		if (result.pv1_state !== undefined && result.pv1_state !== null) {
			await this.setStateChangedAsync("power.pv1State", { val: result.pv1_state, ack: true });
		}

		if (result.pv2_power !== undefined && result.pv2_power !== null) {
			await this.setStateChangedAsync("power.pv2", { val: result.pv2_power, ack: true });
		}
		if (result.pv2_voltage !== undefined && result.pv2_voltage !== null) {
			await this.setStateChangedAsync("power.pv2Voltage", { val: result.pv2_voltage, ack: true });
		}
		if (result.pv2_current !== undefined && result.pv2_current !== null) {
			await this.setStateChangedAsync("power.pv2Current", { val: result.pv2_current, ack: true });
		}
		if (result.pv2_state !== undefined && result.pv2_state !== null) {
			await this.setStateChangedAsync("power.pv2State", { val: result.pv2_state, ack: true });
		}

		if (result.pv3_power !== undefined && result.pv3_power !== null) {
			await this.setStateChangedAsync("power.pv3", { val: result.pv3_power, ack: true });
		}
		if (result.pv3_voltage !== undefined && result.pv3_voltage !== null) {
			await this.setStateChangedAsync("power.pv3Voltage", { val: result.pv3_voltage, ack: true });
		}
		if (result.pv3_current !== undefined && result.pv3_current !== null) {
			await this.setStateChangedAsync("power.pv3Current", { val: result.pv3_current, ack: true });
		}
		if (result.pv3_state !== undefined && result.pv3_state !== null) {
			await this.setStateChangedAsync("power.pv3State", { val: result.pv3_state, ack: true });
		}

		if (result.pv4_power !== undefined && result.pv4_power !== null) {
			await this.setStateChangedAsync("power.pv4", { val: result.pv4_power, ack: true });
		}
		if (result.pv4_voltage !== undefined && result.pv4_voltage !== null) {
			await this.setStateChangedAsync("power.pv4Voltage", { val: result.pv4_voltage, ack: true });
		}
		if (result.pv4_current !== undefined && result.pv4_current !== null) {
			await this.setStateChangedAsync("power.pv4Current", { val: result.pv4_current, ack: true });
		}
		if (result.pv4_state !== undefined && result.pv4_state !== null) {
			await this.setStateChangedAsync("power.pv4State", { val: result.pv4_state, ack: true });
		}
	},
